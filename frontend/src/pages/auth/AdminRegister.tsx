...
const AdminRegister = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (step === 'start') {
        await adminAuthAPI.registerStart(formData);
        setStep('verify');
        return;
      }
      const response = await adminAuthAPI.registerVerify({ email: formData.email, otp });
      localStorage.setItem('access_token', response.data.access_token);
      localStorage.setItem('user_type', 'admin');
      navigate('/admin/dashboard');
    } catch (error: any) {
      setErrorMsg(error?.response?.data?.detail || 'Admin registration failed');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AuthLayout
      ...
      subtitle={step === 'start' ? 'Create your admin account to manage EV stations' : 'Enter the OTP sent to your email'}
    >
      {/* ── Google at the top ── */}
      {googleClientId ? (
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <>
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="input-clean pl-10" placeholder="Admin Name" required />
              </div>
            </div>
        ...
        <button type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2">
          {loading ? <LoadingSpinner size="sm" /> : step === 'start' ? 'Send OTP' : 'Verify & Create Admin Account'}
        </button>

        <p className="text-center text-sm text-muted-foreground pt-1">
          Already have an admin account?{' '}
          <Link to="/admin/login" className="text-primary hover:text-primary/80 font-semibold underline underline-offset-2">Sign in</Link>
        </p>

        <div className="pt-3 border-t border-border">
          <Link to="/register" className="block text-center text-sm text-muted-foreground hover:text-foreground transition-colors">
