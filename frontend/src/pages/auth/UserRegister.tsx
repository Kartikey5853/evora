...
const UserRegister = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (step === 'start' && formData.password !== formData.confirmPassword) {
    ...
    try {
      if (step === 'start') {
        await authAPI.registerStart({ name: formData.name, email: formData.email, password: formData.password });
        setStep('verify');
        return;
      }
      const response = await authAPI.registerVerify({ email: formData.email, otp });
  ...
  const handleGoogleCredential = async (credential: string) => {
    ...
    setErrorMsg(null);
    try {
      const response = await authAPI.googleAuth({ credential, role: 'user' });
      localStorage.setItem('access_token', response.data.access_token);
      navigate('/setup-profile');
    } catch (error: any) {
      setErrorMsg(error?.response?.data?.detail || 'Google signup failed');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AuthLayout
      ...
      subtitle={step === 'start' ? 'Sign up to start your EV charging journey' : 'Enter the OTP sent to your email'}
    >
      {/* ── Google at the top ── */}
      {googleClientId ? (
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <>
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="input-clean pl-10" placeholder="John Doe" required />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="input-clean pl-10" placeholder="you@example.com" required />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <input type={showConfirmPassword ? 'text' : 'password'} value={formData.confirmPassword} onChange={(e) => setFormData({ ...formData, confirmPassword: e.target.value })} className="input-clean pl-10 pr-11" placeholder="••••••••" required />
        ...
        <button type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2">
          {loading ? <LoadingSpinner size="sm" /> : step === 'start' ? 'Send OTP' : 'Verify & Create Account'}
        </button>

        <p className="text-center text-sm text-muted-foreground pt-1">
          Already have an account?{' '}
          <Link to="/login" className="text-primary hover:text-primary/80 font-semibold underline underline-offset-2">Sign in</Link>
        </p>
      </form>
    </AuthLayout>
  );
