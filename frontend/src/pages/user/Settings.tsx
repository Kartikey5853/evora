...
const UserSettings = () => {
  ...
  const [twoFactorEnabled, setTwoFactorEnabled] = useState(localStorage.getItem('two_factor_enabled') === 'true');

  useEffect(() => {
    localStorage.setItem('two_factor_enabled', String(twoFactorEnabled));
  }, [twoFactorEnabled]);
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                When enabled, login requires an OTP.
              </p>
            </div>
